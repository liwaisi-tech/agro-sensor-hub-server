...
class SensorActivityService:
    ...
    def __init__(self, sensor_activity_repository: SensorActivityRepository, device_service: DeviceService):
        self.repository = sensor_activity_repository
        self.device_service = device_service
    
    def _ensure_device_exists(self, db: Session, mac_address: str, zone: Optional[str]) -> DeviceResponse:
        """
    ...
    def _update_device_zone(self, db: Session, device: DeviceResponse, new_zone: str) -> None:
        """
        ...
        if device.name != new_zone:
            self.device_service.update_device(db, DeviceCreate(
    ...
    def create(self, db: Session, activity_create: SensorActivityCreate) -> SensorActivityResponse:
        ...
        try:
            ...
            device = self._ensure_device_exists(db, activity_create.mac_address, activity_create.zone)
            ...
            raise HTTPException(status_code=500, detail=f"Error creating sensor activity: {str(e)}")
    ...
    def get_filtered_list(
        ...
        try:
            ...
            if not activities:
                raise HTTPException(status_code=404, detail="No sensor activities found")
            return activities
        except HTTPException as he:
            raise he
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Error retrieving sensor activities: {str(e)}")
    ...
    def get_by_id(self, db: Session, activity_id: int) -> SensorActivityResponse:
        ...
        try:
            ...
            if not activity:
                raise HTTPException(status_code=404, detail=f"Sensor activity with id {activity_id} not found")
            return activity
        except HTTPException as he:
            raise he
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Error retrieving sensor activity: {str(e)}")

    def get_latest_by_mac_address(self, db: Session, mac_address: str) -> SensorActivityResponse:
        """
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Error retrieving latest sensor activity: {str(e)}")
