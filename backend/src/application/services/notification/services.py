...
class NotificationService:
    ...
    def create(self, db: Session, notification_create: NotificationCreate) -> NotificationResponse:
        """
    ...
    def update_read_status(self, db: Session, notification_id: int, is_read: bool = True) -> NotificationResponse:
        """
        ...
        """
        try:
            notification = self.repository.update_read_status(db, notification_id, is_read)
            if not notification:
                raise HTTPException(
                    status_code=404,
            ...
            raise HTTPException(status_code=500, detail=f"Error updating notification read status: {str(e)}")
    ...
    def get_by_mac_address(
        ...
        """
        try:
            notifications = self.repository.get_by_mac_address(db, mac_address, skip, limit)
            if not notifications:
                raise HTTPException(
                    status_code=404,
